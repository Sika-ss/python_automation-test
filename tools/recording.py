...
class RecordCases:
    ...
    def __init__(self, filter_url: str, filename: str = "data/price_cloud.xlsx", sheet_name: str = "price_cloud"):
    ...
    def response(self, flow: mitmproxy.http.HTTPFlow):
        ...
        if url.find('.css') == -1 and url.find('.js') == -1 and url.find(self.filter_url) != -1:
            ...
            DoLogs(__name__).mylog.info("抓取的请求头: {}".format(flow.request))
            DoLogs(__name__).mylog.info("抓取的响应头: {}".format(flow.response))
            case = [
                case_id,
                None,
                title,
...
addons = [RecordCases('admin-price-cloud-stg.songmao.tech/')]  # 不同的项目组修改对应的域名
